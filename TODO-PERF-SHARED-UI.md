...
- [x] **T2.4 Long-op progress** — `UI.progress()` (chunked + yield): label 240 copies, CSV import, bulk stock. **Test:** `tools/test_progress.js` (main thread block < 100ms). **~60 min** ✔ (v2.30.5) — `UI.progress(title, total)` card (right-bottom, role=progressbar, set/step/label/done/fail) + `UI.progress.run(items, fn, {chunk, shouldStop})` chunked-yield runner (24/chunk + setTimeout(0) → sync bursts bound). Wired: Print.labelsHtml DONO copies (App_Core + App_Barcode — 48+ labels par progress + har 24 par yield; CSV import browser-loop hai hi nahi — backend PriceImport). **Test:** `tools/test_progress.js` 20/0 GREEN (burst≤chunk, yield-proof, shouldStop, fail-rethrow, 120-label sheet, chhoti sheet no-progress).
...
- [x] **T3.5 Data-change reactivity sweep** — jo tables/sections store change par update nahi hote unhe `Store.sub` par wire karna. **Test:** `tools/test_data_aware_sweep.js`. **~60 min** ✔ (v2.30.5) — SHARED ENGINE pehle: `Store.sub/unsub/emit` (App_Core Store pub/sub) + `API.call` har write par `Store.emit('data', {action})` (single point, `_isWrite` verbs) + `UI2.table(load:)` LIVE wiring (debounced 400ms auto-reload; detached-DOM skip; `live: false` opt-out). **BONUS bug-fix (sweep ne pakra):** demo-mock `customers.save` record list me push hi nahi karta tha (sirf return) — ab persist hota hai. NOTE (baqi, jaan-boojh kar nahi chhera): mock `items.list` first-100 par capped hai (bara catalog) — alag slice. **Test:** `tools/test_data_aware_sweep.js` 13/0 GREEN (pub/sub contract · write-emit/read-no-emit · auto-reload + naya row · live:false · debounce 3→1 · detached skip · zero errors).
...
- [x] **T4.4 App-wide apply** ✔ (v2.30.5) — 10 ad-hoc display sites `DT.format`/`fmt.date`/`fmt.dt` par: POS2 subtitle+footer clock, Core lastSync, Demand PO-notes, AIConfig training-history stamp, Screens2 prevDate×2, Pwa_POS receipt+history, Pwa_Warehouse GRN date. PWA ke liye `PWA.fmtDT` (Pwa_Shell — deterministic policy, localStorage dt-override hook). Audit ab GATE hai (`tools/audit_datetime.js` → `gate`: dateLocale/stampDisplay/adhocDate = **0/0/0**; number-formatting en-US, type:'date' input values, ISO payloads, engine fallbacks exempt). **Carry-over (kholi hui):** PWA settings-sync UI — jab ho to PWA.fmtDT khud dt-policy utha lega.

### W5 — VISIBILITY + ROLE/SCOPE (P2+P3+security) — target **v2.26.1**
- [x] **T5.1 `Vis` engine** ✔ SHIPPED v2.28/2.29 `Fields.gs` naam se (TODO-tick reh gaya tha): `Fields.can(session, family)` + `App.field(family)`/`App.fieldFilter` (App_Core) + UI2.table/form `family:` single-point filter + `PWA.field` (Pwa_Shell). Scope: `Auth.canAccessLocation` (row-level, pehle se). **Test:** `tools/test_field_visibility.js` (yahi contract — 58/0 GREEN).
- [x] **T5.2 Sensitive registry** ✔ `Fields.FAMILIES` (6: cost/margin/contact/finance/supplier/notes → col-regex + perm key) + catalog me `field.*` keys (6) + "fields" perm-group. **Test:** test_field_visibility sections F (registry coverage).
- [x] **T5.3 BACKEND enforcement** ✔ `Fields.wrap()` (response strip) + `Fields.scrub()` (write-guard) Code.gs `api()` dispatcher me CENTRAL (har route) — payload-level assertions gate me: cashier response me costPrice column hi nahi, hidden-field write reject, GRN cost scrub-safe, kill-switch. Demo-mock parity (v2.30.5): ROLE_PERMS me field.* keys + `fields.matrix` mirror add.
- [x] **T5.4 Scope-based data** ✔ `Auth.canAccessLocation` + session.locationIds se lists scoped (sales/stock/customers) — roles_matrix + e2e gates cover.
- [x] **T5.5 Settings UI + live role-matrix** ✔ (v2.30.5) Users & Security me 5th tab **"Field visibility"**: families × roles live matrix (`fields.matrix` se, code-static role perms + overrides), bilingual note (extra/denied perms ka rasta). Per-role perm toggles pehle se Permissions tab me (v2.5) — field groups usi me. Gate F: render + parity assertions.
- NOTE: `tools/audit_field_visibility.js` ka per-route "module-perm" metric PURANI design hai (central Fields.wrap superseded) — audit me note laga diya.
...
- [x] **T13.2 B§11 Price-variation signals (supplier)** ✔ (v2.30.4) — audit: party
      history+totals **pehle se** (Ledger tab + Opening/Billed/Received/Closing KPIs + item
      History tab: stock.moves + audit.trail) — koi dobara kaam nahi. Asli khali: supplier
      ki rate-farq ka koi view nahi tha. **NAYA:** backend `purchase.supplierPriceSignals`
      (Purchase.gs — GRN/GRNItems se har item ki aakhri 2 GRN rates, posted-only, flat
      skip, |%| desc, top 20 — real data only) + route + supplier detail me **Price signals
      tab** (sirf SUPPLIER; up err-badge / down ok-badge, prev/last dates).
