...
## Round-17 — EN SLICE-6 + WINDOWS-DEV + v2.31.5 (2026-09-26)
- [x] windows-dev/ dev-kit (Windows 10/11 64-bit + VS Code): setup/serve/run-test/run-all/verify/package/check/i18n/push/ftp-backup/snapshot wrappers + vscode-tasks — push hua
- [x] FTP backup-cycle: 22 files uploaded+size-verified (zips/logs/archive/bundle/2 snapshots) → phir local deletes (repo 178M→163M); .netrc snapshot-excluded → har session mein re-create
